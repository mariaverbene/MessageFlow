...
public class Config {
    ...
    @Bean
    public IntegrationFlow topicFlow1() {              //proceed messages from unregistered users -> in topic2
        return IntegrationFlows
                .from(channelPubSub())
                .handle(unRegUsers)
    ...
    @Bean
    public IntegrationFlow topicFlow2() {              //proceed messages from registered users -> collection -> database
        return IntegrationFlows
                .from(channelPubSub())
                .handle(regUsers)
                .get();
